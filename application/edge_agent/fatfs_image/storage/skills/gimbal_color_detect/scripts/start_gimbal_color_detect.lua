...
local function update_servo_target_from_detection(result)
    ...
    local err_x = result.cx - (result.width / 2)
    local err_y = result.cy - (result.height / 2)
...
local function run()
    ...
    while not deadline_s or os.time() < deadline_s do
        ...
        if ctx.perf_log_every_n > 0 and perf_frames >= ctx.perf_log_every_n then
            ...
            print(string.format(
                "[gimbal_color_detect] perf fps=%.1f avg_ms get=%.1f convert=%.1f detect=%.1f display=%.1f loop=%.1f frame_ts=%.1f",
                fps,
